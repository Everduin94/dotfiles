...
M.autolist = {
  ...
  config = function()
    ...
    local function set_autolist_keymaps(buffer)
      local opts = { buffer = buffer }
      local expr_opts = { buffer = buffer, expr = true }
      ...
      vim.keymap.set("n", "<leader>cn", require("autolist").cycle_next_dr, expr_opts)
      vim.keymap.set("n", "<leader>cp", require("autolist").cycle_prev_dr, expr_opts)
    ...
    set_autolist_keymaps(0)
    vim.api.nvim_create_autocmd("FileType", {
      pattern = { "markdown", "text", "tex", "plaintex", "norg" },
      callback = function(args)
        set_autolist_keymaps(args.buf)
      end,
    })
  end,
}
