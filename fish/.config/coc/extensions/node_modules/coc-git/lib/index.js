...
var DocumentManager = class {
  ...
  async defineSigns() {
    ...
    let numhl = signcolumn == "number" && import_coc8.workspace.isNvim && import_semver.default.gte(import_coc8.workspace.env.version, "v0.3.2");
  ...
  async browser(action = "open", range) {
    ...
    if (browserRemoteName.length > 0) {
      if (names.includes(browserRemoteName)) {
        ...
        import_coc11.window.showMessage("Configured git.browserRemoteName missing from remote list", "warning");
        return;
      }
    }
    let urls = [];
...
async function activate(context) {
  ...
  subscriptions.push(import_coc14.workspace.registerKeymap(["n"], "git-keepboth", async () => {
  ...
  subscriptions.push(import_coc14.commands.registerCommand("git.keepIncoming", async () => {
    await manager.keepIncoming();
  }));
  subscriptions.push(import_coc14.commands.registerCommand("git.keepBoth", async () => {
    await manager.keepBoth();
  }));
