...
class JSDebugMessage extends __1.DebugMessage {
    ...
    msg(textEditor, document, selectedVar, lineOfSelectedVar, wrapLogMessage, logMessagePrefix, quote, addSemicolonInTheEnd, insertEnclosingClass, insertEnclosingFunction, delemiterInsideMessage, includeFileNameAndLineNum, tabSize) {
        ...
        if (/\){.*}/.test(previousMsgLogLine.text.replace(/\s/g, ""))) {
            const textBeforeClosedFunctionParenthesis = previousMsgLogLine.text.split(")")[0];
            textEditor.delete(previousMsgLogLine.rangeIncludingLineBreak);
            textEditor.insert(new vscode.Position(lineOfLogMsg >= document.lineCount
                ? document.lineCount
                : lineOfLogMsg, 0), `${textBeforeClosedFunctionParenthesis}){\n${lineOfLogMsg === document.lineCount ? "\n" : ""}${spacesBeforeMsg}${debuggingMsg}\n${spacesBeforeMsg}}\n`);
        }
        else {
            textEditor.insert(new vscode.Position(lineOfLogMsg >= document.lineCount
                ? document.lineCount
