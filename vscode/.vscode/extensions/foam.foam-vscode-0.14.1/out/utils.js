 ...
 */
exports.createNoteFromPlaceholder = async (placeholder) => {
    var _a;
    const basedir = vscode_1.workspace.workspaceFolders.length > 0
        ? vscode_1.workspace.workspaceFolders[0].uri
        : ((_a = vscode_1.window.activeTextEditor) === null || _a === void 0 ? void 0 : _a.document.uri) ? foam_core_1.URI.getDir(vscode_1.window.activeTextEditor.document.uri)
            : null;
    if (isSome(basedir)) {
        const target = vsc_utils_1.toVsCodeUri(foam_core_1.URI.createResourceUriFromPlaceholder(basedir, placeholder));
        await vscode_1.workspace.fs.writeFile(target, new util_1.TextEncoder().encode(''));
        return target;
    }
    return null;
};
