...
const template_frontmatter_parser_1 = require("./template-frontmatter-parser");
describe('extractFoamTemplateFrontmatterMetadata', () => {
    test('Returns an empty object if there is not frontmatter', () => {
        ...
        const result = template_frontmatter_parser_1.extractFoamTemplateFrontmatterMetadata(input);
...
`;
        ...
        expectedMetadata.set('filepath', 'journal/$CURRENT_YEAR-$CURRENT_MONTH-$CURRENT_DATE_$FOAM_TITLE.md');
...
# $FOAM_TITLE`;
        const result = template_frontmatter_parser_1.removeFoamMetadata(input);
        expect(result).toEqual(expected);
    });
});
//# sourceMappingURL=template-frontmatter-parser.test.js.map