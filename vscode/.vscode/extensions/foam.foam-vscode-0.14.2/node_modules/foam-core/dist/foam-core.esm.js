import { win32, posix as posix$1, parse } from 'path';
...
var URI = /*#__PURE__*/function () {
  ...
  URI.joinPath = function joinPath(uri) {
    ...
    if (isWindows && uri.scheme === 'file') {
      ...
      newPath = URI.file((_paths$win = win32).join.apply(_paths$win, [URI.toFsPath(uri)].concat(pathFragment))).path;
    } else {
      var _paths$posix;

      newPath = (_paths$posix = posix$1).join.apply(_paths$posix, [uri.path].concat(pathFragment));
    }
...
var findAllFiles = /*#__PURE__*/promisify(glob);
/**
  ...
  _proto2.read = function read(uri) {
    try {
      return Promise.resolve(_catch(function () {
        return Promise.resolve(fs.promises.readFile(URI.toFsPath(uri))).then(function (_fs$promises$readFile) {
...
var extractHashtags = function extractHashtags(text) {
  return isSome(text) ? new Set(Array.from(text.matchAll(HASHTAG_REGEX), function (m) {
    return m[2].trim();
  })) : new Set();
};
var extractTagsFromProp = function extractTagsFromProp(prop) {
  var text = Array.isArray(prop) ? prop.join(' ') : prop;
  return isSome(text) ? new Set(Array.from(text.matchAll(WORD_REGEX)).map(function (m) {
    return m[2].trim();
  })) : new Set();
};
...
var pathToResourceId = function pathToResourceId(pathValue) {
  var _path$parse = parse(pathValue),
      ext = _path$parse.ext;
...
var pathToResourceName = function pathToResourceName(pathValue) {
  return parse(pathValue).name.toLowerCase();
};
...
var FoamWorkspace = /*#__PURE__*/function () {
  ...
  _proto.set = function set(resource) {
    ...
    this.resourcesByName[name] = (_this$resourcesByName = this.resourcesByName[name]) !== null && _this$resourcesByName !== void 0 ? _this$resourcesByName : [];
  ...
  _proto["delete"] = function _delete(uri) {
    var _this$resourcesByName2, _this$resourcesByName3;

    var id = uriToResourceId(uri);
    var deleted = this.resources[id];
    delete this.resources[id];
    var name = uriToResourceName(uri);
    this.resourcesByName[name] = (_this$resourcesByName2 = (_this$resourcesByName3 = this.resourcesByName[name]) === null || _this$resourcesByName3 === void 0 ? void 0 : _this$resourcesByName3.filter(function (resId) {
      return resId !== id;
  ...
  _proto.exists = function exists(uri) {
    return !URI.isPlaceholder(uri) && isSome(this.resources[uriToResourceId(uri)]);
  };
  ...
  _proto.find = function find(resourceId, reference) {
    var _this$resources$uriTo, _this$resources$uriTo2;
    ...
    switch (refType) {
      ...
      case 'key':
        ...
        var paths = this.resourcesByName[name];

        if (isNone(paths) || paths.length === 0) {
          paths = this.resourcesByName[resourceId];
        }
      ...
      case 'absolute-path':
        ...
        return (_this$resources$uriTo = this.resources[uriToResourceId(resourceUri)]) !== null && _this$resources$uriTo !== void 0 ? _this$resources$uriTo : null;
      ...
      case 'relative-path':
        ...
        return (_this$resources$uriTo2 = this.resources[uriToResourceId(targetUri)]) !== null && _this$resources$uriTo2 !== void 0 ? _this$resources$uriTo2 : null;
...
var FoamGraph = /*#__PURE__*/function () {
  ...
  _proto.getAllNodes = function getAllNodes() {
    return [].concat(Object.values(this.placeholders), this.workspace.list().map(function (r) {
      return r.uri;
    }));
  };
  ...
  _proto.getConnections = function getConnections(uri) {
    return [].concat(this.links[uri.path] || [], this.backlinks[uri.path] || []);
  };

  _proto.getLinks = function getLinks(uri) {
    var _this$links$uri$path;

    return (_this$links$uri$path = this.links[uri.path]) !== null && _this$links$uri$path !== void 0 ? _this$links$uri$path : [];
  };

  _proto.getBacklinks = function getBacklinks(uri) {
    var _this$backlinks$uri$p;

    return (_this$backlinks$uri$p = this.backlinks[uri.path]) !== null && _this$backlinks$uri$p !== void 0 ? _this$backlinks$uri$p : [];
  }
  /**
  ...
  FoamGraph.fromWorkspace = function fromWorkspace(workspace, keepMonitoring) {
    ...
    Object.values(workspace.list()).forEach(function (resource) {
  ...
  _proto.updateLinksRelatedToAddedResource = function updateLinksRelatedToAddedResource(resource) {
    ...
    if (name in this.placeholders) {
      var _this$backlinks$place;

      var placeholder = this.placeholders[name];
      delete this.placeholders[name];
      var resourcesToUpdate = (_this$backlinks$place = this.backlinks[placeholder.path]) !== null && _this$backlinks$place !== void 0 ? _this$backlinks$place : [];
  ...
  _proto.updateLinksForResource = function updateLinksForResource(oldResource, newResource) {
    ...
    if (oldResource.type === 'note' && newResource.type === 'note') {
      var patch = diff(oldResource.links, newResource.links, isEqual);
  ...
  _proto.updateLinksRelatedToDeletedResource = function updateLinksRelatedToDeletedResource(resource) {
    var _this$links$uri$path2,
        _this3 = this,
        _this$backlinks$uri$p2;

    var uri = resource.uri; // remove forward links from old resource

    var resourcesPointedByDeletedNote = (_this$links$uri$path2 = this.links[uri.path]) !== null && _this$links$uri$path2 !== void 0 ? _this$links$uri$path2 : [];
    ...
    var notesPointingToDeletedResource = (_this$backlinks$uri$p2 = this.backlinks[uri.path]) !== null && _this$backlinks$uri$p2 !== void 0 ? _this$backlinks$uri$p2 : [];
  ...
  _proto.connect = function connect(source, target, link) {
    var _this$links$source$pa, _this$backlinks$targe;
    ...
    this.links[source.path] = (_this$links$source$pa = this.links[source.path]) !== null && _this$links$source$pa !== void 0 ? _this$links$source$pa : [];
    this.links[source.path].push(connection);
    this.backlinks[target.path] = (_this$backlinks$targe = this.backlinks[target.path]) !== null && _this$backlinks$targe !== void 0 ? _this$backlinks$targe : [];
  ...
  _proto.disconnect = function disconnect(source, target, link) {
    var _this$links$source$pa2, _this$links$source$pa3, _this$backlinks$targe2, _this$backlinks$targe3;
    ...
    this.links[source.path] = (_this$links$source$pa2 = (_this$links$source$pa3 = this.links[source.path]) === null || _this$links$source$pa3 === void 0 ? void 0 : _this$links$source$pa3.filter(connectionsToKeep)) !== null && _this$links$source$pa2 !== void 0 ? _this$links$source$pa2 : [];

    if (this.links[source.path].length === 0) {
      delete this.links[source.path];
    }

    this.backlinks[target.path] = (_this$backlinks$targe2 = (_this$backlinks$targe3 = this.backlinks[target.path]) === null || _this$backlinks$targe3 === void 0 ? void 0 : _this$backlinks$targe3.filter(connectionsToKeep)) !== null && _this$backlinks$targe2 !== void 0 ? _this$backlinks$targe2 : [];
  ...
  _proto.resolveResource = function resolveResource(resource) {
    ...
    delete this.links[resource.uri.path]; // prettier-ignore
...
var FoamTags = /*#__PURE__*/function () {
  ...
  FoamTags.fromWorkspace = function fromWorkspace(workspace, keepMonitoring) {
    ...
    var tags = new FoamTags(workspace);
    Object.values(workspace.list()).forEach(function (resource) {
  ...
  _proto.addResourceFromTagIndex = function addResourceFromTagIndex(resource) {
    ...
    resource.tags.forEach(function (tag) {
      var _this$tags$get, _this$tags$get2;

      _this.tags.set(tag, (_this$tags$get = _this.tags.get(tag)) !== null && _this$tags$get !== void 0 ? _this$tags$get : []);

      (_this$tags$get2 = _this.tags.get(tag)) === null || _this$tags$get2 === void 0 ? void 0 : _this$tags$get2.push({
        uri: resource.uri
      });
    });
  };
  ...
  _proto.removeResourceFromTagIndex = function removeResourceFromTagIndex(resource) {
    ...
    resource.tags.forEach(function (tag) {
      if (_this2.tags.has(tag)) {
        ...
        var remainingLocations = (_this2$tags$get = _this2.tags.get(tag)) === null || _this2$tags$get === void 0 ? void 0 : _this2$tags$get.filter(function (meta) {
          return !URI.isEqual(meta.uri, resource.uri);
        });
...
var tagsPlugin = {
  name: 'tags',
  onWillVisitTree: function onWillVisitTree(tree, note) {
  ...
  onDidFindProperties: function onDidFindProperties(props, note) {
    ...
    yamlTags.forEach(function (tag) {
      return note.tags.add(tag);
    });
  }
};
var titlePlugin = {
...
var handleError = function handleError(plugin, fnName, uri, e) {
  ...
  Logger.warn("Error while executing [" + fnName + "] in plugin [" + name + "] for file [" + (uri === null || uri === void 0 ? void 0 : uri.path) + "]", e);
};
      ...
      visit(tree, function (node) {
        if (node.type === 'yaml') {
          try {
            ...
            var yamlProperties = (_parseYAML = parse$1(node.value)) !== null && _parseYAML !== void 0 ? _parseYAML : {};
            ...
            for (var i = 0, len = plugins.length; i < len; i++) {
              try {
                ...
                (_plugins$i$onDidFindP = (_plugins$i = plugins[i]).onDidFindProperties) === null || _plugins$i$onDidFindP === void 0 ? void 0 : _plugins$i$onDidFindP.call(_plugins$i, yamlProperties, note);
              } catch (e) {
...
function createMarkdownReferences(workspace, noteUri, includeExtension) {
  ...
  if ((source === null || source === void 0 ? void 0 : source.type) !== 'note') {
    console.warn("Note " + noteUri + " note found in workspace when attempting to generate markdown reference list");
    return [];
  }
...
var createConfigFromFolders = function createConfigFromFolders(workspaceFolders, options) {
  ...
  var workspaceConfig = workspaceFolders.reduce(function (acc, f) {
    return merge(acc, parseConfig(URI.joinPath(f, 'config.json')));
  ...
  var settings = merge(workspaceConfig, userConfig);
...
export { BaseLogger, Emitter, Event, FileDataStore, FoamGraph, FoamTags, FoamWorkspace, LINK_REFERENCE_DEFINITION_FOOTER, LINK_REFERENCE_DEFINITION_HEADER, Logger, MarkdownResourceProvider, Matcher, Position, Range, Resource, URI, applyTextEdit, bootstrap, createConfigFromFolders, createMarkdownParser, createMarkdownReferences, generateHeading, generateLinkReferences, getKebabCaseFileName, isDisposable, stringifyMarkdownLinkReferenceDefinition };
//# sourceMappingURL=foam-core.esm.js.map
