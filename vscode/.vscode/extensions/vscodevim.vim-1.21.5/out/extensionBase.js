...
async function getAndUpdateModeHandler(forceSyncAndUpdate = false) {
    ...
    const [curHandler, isNew] = await modeHandlerMap_1.ModeHandlerMap.getOrCreate(activeEditorId);
    if (isNew) {
...
async function activate(context, handleLocal = true) {
    ...
    await Promise.all([commandLine_1.commandLine.load(context), globalState_1.globalState.load(context)]);
    ...
    registerEventListener(context, vscode.workspace.onDidCloseTextDocument, async (closedDocument) => {
        ...
        for (const editorIdentity of modeHandlerMap_1.ModeHandlerMap.getKeys()) {
    ...
    registerEventListener(context, vscode.window.onDidChangeTextEditorSelection, async (e) => {
        ...
        const mh = modeHandlerMap_1.ModeHandlerMap.get(editorIdentity_1.EditorIdentity.fromEditor(vscode.window.activeTextEditor));
        if (mh === undefined) {
